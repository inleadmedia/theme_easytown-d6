 ...
 */
function easytown_ting_search_form($form){
  $form['submit']['#type']  = "submit" ;
  $form['submit']['#src']   = drupal_get_path('theme','easytown')."/images/searchbutton.png";
 ...
 */
function easytown_user_login_block($form){
  $form['submit']['#type']  = "submit" ;
  $form['submit']['#src']   = drupal_get_path('theme','easytown')."/images/accountlogin.png";
  $form['submit']['#attributes']['class']   = "";
 ...
 */
function easytown_menu_item_link($link) {
 ...
 */
function easytown_preprocess_page(&$variables) {
	//Magic function to override module js with .info js files :-)
	$info = file_get_contents(drupal_get_path('theme','easytown').'/easytown.info');
	...
	if (0 < count($matches[1])) {
		foreach ($matches[1] AS $key => $value) {
			...
			$replaceWith = '<script type="text/javascript" defer="defer" src="'.base_path().drupal_get_path('theme','easytown').'/'.$value.'$1"></script>';
 ...
 */
function easytown_preprocess_node(&$variables) {
 ...
 */
function easytown_preprocess_ding_panels_content_library_location(&$variables) {
  $node = $variables['node'];
 ...
 */
function easytown_preprocess_ding_panels_content_library_title(&$variables) {
